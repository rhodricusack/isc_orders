...
amp_noise_mean = 2
amp_noise_std = 1
amp_order_factor = [1,1,1,1.2,0.8,0.7] # some orders have less signal and more noise
...
for amp_sig in np.arange(0,1,0.2):
    ...
    for simind in range(nsim):
        ...
        for orderind in range(norder):
            sig =  amp_order_factor[orderind] * amp_sig * np.random.randn(ntp) # Signal for a given orderel (i.e., order)        
            for subjind in range(nsubj):
                ...
                y[:,orderind,subjind] = np.random.randn(ntp) / amp_order_factor[orderind] * subjnoise + sig # signal same for all subjects
        ...
        if not simind:
            ...
            plt.title(f'Example null distribution (observed={np.mean(observed)})')
            plt.savefig(f'null_dist_method1.jpg')

        # METHOD 2 - average DISTRIBUTION
        distribution = np.mean(np.array(distribution), axis=1)
        # Shift bootstrap distribution to 0 for hypothesis test
        observed2 = np.mean(compute_summary_statistic(simisc, 'median', axis=0))
    ...
    print(f' SW individual false positive rate (alpha=0.05) {np.mean(np.array(allp_sw)<0.05, axis=0)}')
    print(f' SW combined method 1 false positive rate (alpha=0.05) {np.mean(np.array(allp_combined_method1_sw)<0.05, axis=0)}')
    print(f' SW combined method 2 false positive rate (alpha=0.05) {np.mean(np.array(allp_combined_method2_sw)<0.05, axis=0)}')

    df = df.append({'amp_sig':amp_sig, 'fpr_ind': np.mean(np.array(allp_sw)<0.05, axis=0), 'fpr_comb1': np.mean(np.array(allp_combined_method1_sw)<0.05, axis=0), 'fpr_comb2': np.mean(np.array(allp_combined_method2_sw)<0.05, axis=0)}, ignore_index=True)
